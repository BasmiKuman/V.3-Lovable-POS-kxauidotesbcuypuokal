...
export const BottomNav = ({ isAdmin }: BottomNavProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border shadow-lg safe-bottom">
      <div className="overflow-x-auto scrollbar-none">
        <div className="flex items-center justify-evenly h-14 sm:h-16 max-w-screen-xl mx-auto">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                className={cn(
                  "flex flex-col items-center justify-center flex-1 h-full min-w-[3.5rem] sm:min-w-[5rem] max-w-[7rem] px-1 space-y-0.5 sm:space-y-1 rounded-lg transition-all",
                  isActive
                    ? "text-primary"
              ...
              >
                ...
                <span className="text-[0.65rem] sm:text-xs font-medium text-center leading-tight truncate">{item.label}</span>
              </Link>
            );
          })}
        </div>
